...
public class sikrsun_chackle extends BaseShipSystemScript {
	public static Object KEY_SHIP = new Object();
	public static Object KEY_TARGET = new Object();
	
	public static float ACC_MULT = 0.8f;
	...
	public void apply(MutableShipStatsAPI stats, final String id, State state, float effectLevel) {
		...
		final String targetDataKey = ship.getId() + "sikrsun_chackle_target";
		
		Object targetDataObj = (Object) Global.getCombatEngine().getCustomData().get(targetDataKey);
				...
				public void advance(float amount, List<InputEventAPI> events) {
					...
					if (targetData.target == Global.getCombatEngine().getPlayerShip()) { 
						Global.getCombatEngine().maintainStatusForPlayerShip(KEY_TARGET, 
								...
								"" + (int)((1f - targetData.currAccMult) * 100f) + "% acceleration lost", true);
					}
					
					if (targetData.currAccMult <= 1f || !targetData.ship.isAlive()) {
                        ...
                        targetData.target.getMutableStats().getAcceleration().modifyMult(targetDataKey, targetData.currAccMult);
                        targetData.target.getMutableStats().getTurnAcceleration().modifyMult(targetDataKey, targetData.currAccMult);
					}
				}
			};
	...
	public StatusData getStatusData(int index, State state, float effectLevel) {
		if (effectLevel > 0) {
			if (index == 0) {
				float damMult = 1f + (1f - ACC_MULT) * effectLevel;
				return new StatusData("" + (int)((damMult) * 100f) + "% acceleration neutralised", false);
			}
		}
		return null;
	}
